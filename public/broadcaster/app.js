...
async function requestMedia(constraints, timeoutMs) {
  let timedOut = false;
...
async function tryConnectMic() {
  try {
    // A microphone is optional. Some browsers leave a permission request
    // pending forever, so it must never block the station from going live.
    const micStream=await requestMedia({audio:true,video:false},4000);
    if (!micStream) {
      console.warn("Mic unavailable or permission request timed out.");
      return;
    }
...
async function startPipeline() {
  ...
  if(source==="cam"){
    ...
    const camStream=await requestMedia(cs,8000);
    if (!camStream) throw new Error("Camera permission request timed out.");
